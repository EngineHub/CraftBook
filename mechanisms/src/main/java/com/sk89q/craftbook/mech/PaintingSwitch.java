...
public class PaintingSwitch implements Listener {
    ...
    public void onHeldItemChange(PlayerItemHeldEvent event) {
        ...
        try {
            ...
            AxisAlignedBB bb = AxisAlignedBB.a(Math.min(x1, x2), y1, Math.min(z1, z2), Math.max(x1, x2), y2,
                    Math.max(z1, z2));
            ...
            if (entities.size() == 1 && entities.get(0) instanceof EntityPainting) {
                ...
                EntityPainting newPainting = new EntityPainting(cWorld.getHandle(), oldPainting.x, oldPainting.y,
                        oldPainting.z, oldPainting.direction % 4);

                newPainting.art = oldPainting.art;
                oldPainting.dead = true;

                plugin.getServer().getScheduler().scheduleSyncDelayedTask(plugin, new PaintingRespawner(newPainting,
                        isForwards), 1L);
            }
        } catch (Exception e) {
            return;
    ...
    public class PaintingRespawner implements Runnable {
        ...
        public void run() {
            ...
            if (possiblePaintings.size() != 0) {
                painting.art = possiblePaintings.get((possiblePaintings.indexOf(oldArt) + (reverse.booleanValue() ?
                        -1 : 1) + possiblePaintings.size()) % possiblePaintings.size());
                painting.setDirection(painting.direction);

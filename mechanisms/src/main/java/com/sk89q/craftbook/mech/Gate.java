...
/**
 * Handler for gates. Gates are merely fence blocks. When they are closed or open, a nearby fence will be found, the algorithm will traverse to the
 * top-most connected fence block, and then proceed to recurse to the sides up to a certain number of fences. To the fences that it gets to, it will
...
public class Gate extends AbstractMechanic {
            ...
            if (plugin.getLocalConfiguration().mechSettings.stopDestruction) {
                if (ID == 0 || hasEnoughBlocks(sign, otherSign)) {
                    if (ID == 0 && isValidGateBlock(world.getBlockAt(x, y1, z))) {
                        addBlocks(sign, 1);
                    } else if (ID != 0 && canPassThrough(world.getBlockAt(x, y1, z).getTypeId()) && isValidGateItem(new ItemStack(ID, 1))) {
                        removeBlocks(sign, 1);
                    }
                    world.getBlockAt(x, y1, z).setTypeId(ID);

                    setBlocks(sign, getBlocks(sign, otherSign));
                } else if (!hasEnoughBlocks(sign, otherSign) && isValidGateItem(new ItemStack(ID, 1))) if (player != null) {
            ...
            if (sign.getLine(1).equalsIgnoreCase("[Gate]")) {
                ...
                if (line0 != null && !line0.isEmpty()) {
                    try {
                        ...
                        if (iLine0 != 0 && !isValidGateBlock(iLine0)) { throw new NumberFormatException(); }
                ...
                if (line0 != null && !line0.isEmpty()) {
                    try {
                        if (!isValidGateBlock(Integer.parseInt(line0))) { throw new NumberFormatException(); }
                    } catch (NumberFormatException e) {
                        throw new InvalidMechanismException("Line 1 needs to be a valid block id.");
                    }
                } else {
    ...
    public int getFirstBlock (WorldVector pt) {
        ...
        World world = ((BukkitWorld) pt.getWorld()).getWorld();
        if (!isValidGateBlock(world.getBlockAt(pt.getBlockX(), pt.getBlockY(), pt.getBlockZ()))) return 0;
    ...
    public boolean hasEnoughBlocks (Sign s, Sign other) {
        ...
        return s != null && s.getLine(3).equalsIgnoreCase("infinite") || other != null && other.getLine(3).equalsIgnoreCase("infinite")
                || getBlocks(s, other) > 0;
    }

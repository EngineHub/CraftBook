...
public class HiddenSwitch extends AbstractMechanic {
	...
	public void onRightClick(org.bukkit.event.player.PlayerInteractEvent event) {
		...
		Block testBlock = switchBlock.getFace(event.getBlockFace().getOppositeFace());
	...
	private void toggleSwitches(Block sign, BlockFace direction) {
		Block up = sign.getFace(BlockFace.UP);
		Block down = sign.getFace(BlockFace.DOWN);
		Block adj1, adj2;
		
		// Get perpendicular plane
		if(direction == BlockFace.EAST || direction == BlockFace.WEST){
			adj1 = sign.getFace(BlockFace.NORTH);
			adj2 = sign.getFace(BlockFace.SOUTH);
		}else{
			adj1 = sign.getFace(BlockFace.EAST);
			adj2 = sign.getFace(BlockFace.WEST);
		}

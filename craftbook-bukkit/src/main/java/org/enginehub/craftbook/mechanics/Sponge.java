        ...
        for (int cx = -radius; cx <= radius; cx++) {
            for (int cy = -radius; cy <= radius; cy++) {
                for (int cz = -radius; cz <= radius; cz++) {
                    ...
                    if (circularRadius && !LocationUtil.isWithinSphericalRadius(sponge.getLocation(), event.getToBlock().getLocation(), radius))
                        continue;
        ...
        for (int cx = -radius; cx <= radius; cx++) {
            for (int cy = -radius; cy <= radius; cy++) {
                for (int cz = -radius; cz <= radius; cz++) {
                    ...
                    if (circularRadius && !LocationUtil.isWithinSphericalRadius(water.getLocation(), block.getLocation(), radius))
                        continue;
    ...
    public void addWater(Block block) {
        ...
        for (cy = block.getY() - radius - 1; cy <= block.getY() + radius + 1; cy++) {
            for (cz = block.getZ() - radius - 1; cz <= block.getZ() + radius + 1; cz++) {
                ...
                if (circularRadius && !LocationUtil.isWithinSphericalRadius(water.getLocation(), block.getLocation(), radius + 1.5))
                    continue;
                if (water.getType() == Material.WATER) {

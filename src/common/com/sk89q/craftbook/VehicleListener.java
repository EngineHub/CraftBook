...
public class VehicleListener extends CraftBookDelegateListener {
    ...
    public void onDirectWireInput(Vector pt, boolean isOn, Vector changed) {
        ...
        if (minecartDispensers && type == BlockType.CHEST
                && (CraftBookCore.getBlockID(pt.add(0, -2, 0)) == BlockType.SIGN_POST
                    || CraftBookCore.getBlockID(pt.add(0, -1, 0)) == BlockType.SIGN_POST)) {
            ...
            try {
                ...
                if (push) {
                    int data = CraftBookCore.getBlockData(signPos);
        ...
        // Minecart station
                ...
                && CraftBookCore.getBlockID(pt.add(0, 1, 0)) == BlockType.MINECART_TRACKS
                && (CraftBookCore.getBlockID(pt.add(0, -2, 0)) == BlockType.SIGN_POST
                    || CraftBookCore.getBlockID(pt.add(0, -1, 0)) == BlockType.SIGN_POST)) {
            ...
            Vector motion;
            int data = CraftBookCore.getBlockData(
    ...
    public void onVehiclePositionChange(BaseVehicle vehicle,
        ...
        if (vehicle instanceof Minecart) {
            ...
            int under = CraftBookCore.getBlockID(blockX, blockY - 1, blockZ);
            ...
            if (minecartControlBlocks) {
                ...
                if (under == minecart25xBoostBlock) {
                    Boolean test = RedstoneUtil.testAnyInput(underPt);
                    ...
                    if (test == null || test) {
                        ...
                        if (CraftBookCore.getBlockID(signPos) == BlockType.SIGN_POST
                            ...
                            if (dir != null) {
                                if (CraftBookUtil.isSameSign(minecart.getMotionX(),
                                        dir.getBlockX())) {
                                    reverseX = false;
                                }
                                if (CraftBookUtil.isSameSign(minecart.getMotionZ(),
                                        dir.getBlockZ())) {
                                    reverseZ = false;
                                }
                            }
                        ...
                        if (player != null) {
                            ...
                            if (CraftBookCore.getBlockID(signPos) == BlockType.SIGN_POST
                                ...
                                if (pos != null) {
                                    ...
                                    // Is the spot free?
                                    if (BlockType.canPassThrough(CraftBookCore.getBlockID(pos.add(0, 1, 0)))
                                            && BlockType.canPassThrough(CraftBookCore.getBlockID(pos))) {
                                        loc = new Location(
                                                pos.getBlockX(),
                                                pos.getBlockY(),
                                                pos.getBlockZ());
                    ...
                    if (test == null || test) {
                        ...
                        if (sign != null) {
                            ...
                            int signData = CraftBookCore.getBlockData(
                            ...
                            if (targetTrack != null
                                    && CraftBookCore.getBlockID(targetTrack) == BlockType.MINECART_TRACKS) {
                                CraftBookCore.setBlockData(targetTrack, newData);
                            }
                        }
                    }
            ...
            if (minecartTrackMessages
                    && CraftBookCore.getBlockID(underPt.add(0, -1, 0)) == BlockType.SIGN_POST) {
                Vector signPos = underPt.add(0, -1, 0);
                
                Boolean test = RedstoneUtil.testAnyInput(signPos);
            ...
            if (minecartDispensers) {
                ...
                if (CraftBookCore.getBlockID(pt.add(1, 0, 0)) == BlockType.CHEST
                        && (CraftBookCore.getBlockID(pt.add(-1, 0, 0)) == BlockType.MINECART_TRACKS
                        || CraftBookCore.getBlockID(pt.add(-1, -1, 0)) == BlockType.MINECART_TRACKS
                        || CraftBookCore.getBlockID(pt.add(-1, 1, 0)) == BlockType.MINECART_TRACKS)) {
                    depositPt = pt.add(1, 0, 0);
                } else if (CraftBookCore.getBlockID(pt.add(-1, 0, 0)) == BlockType.CHEST
                        && (CraftBookCore.getBlockID(pt.add(1, 0, 0)) == BlockType.MINECART_TRACKS
                        || CraftBookCore.getBlockID(pt.add(1, -1, 0)) == BlockType.MINECART_TRACKS
                        || CraftBookCore.getBlockID(pt.add(1, 1, 0)) == BlockType.MINECART_TRACKS)) {
                    depositPt = pt.add(-1, 0, 0);
                } else if (CraftBookCore.getBlockID(pt.add(0, 0, 1)) == BlockType.CHEST
                        && (CraftBookCore.getBlockID(pt.add(0, 0, -1)) == BlockType.MINECART_TRACKS
                        || CraftBookCore.getBlockID(pt.add(0, -1, -1)) == BlockType.MINECART_TRACKS
                        || CraftBookCore.getBlockID(pt.add(0, 1, -1)) == BlockType.MINECART_TRACKS)) {
    ...
    public void onVehicleUpdate(BaseVehicle vehicle) {
        ...
        if (vehicle instanceof Minecart) {
                    ...
                    if (test != null) {
                        if (!test) {
                            ...
                            if (cblock != null && cblock instanceof Sign) {
                                ...
                                if (motion != null) {
                                    if (!CraftBookUtil.isSameSign(minecart.getMotionX(), motion.getX())
                                            || minecart.getMotionX() < motion.getX()) {
                                        minecart.setMotionX(motion.getX());
                                    }
                                    if (!CraftBookUtil.isSameSign(minecart.getMotionY(), motion.getY())
                                            || minecart.getMotionY() < motion.getY()) {
                                        minecart.setMotionY(motion.getY());
                                    }
                                    if (!CraftBookUtil.isSameSign(minecart.getMotionZ(), motion.getZ())
            ...
            int block = CraftBookCore.getBlockID(blockX, blockY, blockZ);
    ...
    public boolean onBlockPlace(Player player, Block blockPlaced,
        ...
        if (blockPlaced.getType() == BlockType.MINECART_TRACKS) {
            int under = CraftBookCore.getBlockID(blockPlaced.getX(),
            ...
            if (minecartControlBlocks && under == minecartStationBlock) {
                ...
                boolean needsRedstone = RedstoneUtil.testAnyInput(pt) == null;
    ...
    public boolean onSignChange(Player player, Sign sign) {
        int type = CraftBookCore.getBlockID(
                sign.getX(), sign.getY(), sign.getZ());

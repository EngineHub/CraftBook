...
public class Footprints extends SpongeMechanic {
    ...
    public void onEntityMove(DisplaceEntityEvent.Move.TargetLiving event) {
        ...
        if(event.getTargetEntity().isOnGround())
            event.getTargetEntity().getWorld().spawnParticles(event.getGame().getRegistry().createParticleEffectBuilder(ParticleTypes.FOOTSTEP).build(), event.getTargetEntity().getLocation().getPosition().add(0,0.19,0));
    }
}

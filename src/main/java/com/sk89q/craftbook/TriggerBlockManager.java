...
class TriggerBlockManager {
     ...
     * @param p
     *
     * @return a persistent mechanic if one is triggered by the location; null if one does not already exist
     *         (detection for a potential mechanic that
    ...
    /**
     * Get a list of mechanics that in a specified chunk. Implemented by performing a walk over the entire list of
     * loaded PersistentMechanic in the
     * universe.
     *
     * @param chunk
     *
     * @return a set including every PersistentMechanic with at least one trigger in the given chunk.
     *         (PersistentMechanic with watched blocks in the

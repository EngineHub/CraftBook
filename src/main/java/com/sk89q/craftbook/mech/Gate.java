...
public class Gate extends AbstractMechanic {
	 ...
	 *         otherwise.
	 */
	private boolean recurseColumn(LocalPlayer player, WorldVector pt, Set<BlockVector> visitedColumns,
			Boolean close) {

		World world = ((BukkitWorld) pt.getWorld()).getWorld();
		if (plugin.getConfiguration().gateLimitColumns
				&& visitedColumns.size() > plugin.getConfiguration().gateColumnLimit)
			return false;
	...
	public void onRightClick(PlayerInteractEvent event) {
		...
		if (!plugin.getConfiguration().gateEnabled)
			return;
	...
	public void onBlockRedstoneChange(final SourcedBlockRedstoneEvent event) {
		...
		if (!plugin.getConfiguration().gateAllowRedstone)
			return;
			...
			if (block.getTypeId() == BlockID.WALL_SIGN || block.getTypeId() == BlockID.SIGN_POST) {
				...
				if (state instanceof Sign) {
					...
					if (sign.getLine(1).equalsIgnoreCase("[Gate]")
						...
						// for the blocks that make up the movable
						// parts of the gate until we're running the event
						// later... so the factory can succeed even if
						// the signpost doesn't actually operate any gates
		...
		public Gate detect(BlockWorldVector pt, LocalPlayer player, ChangedSign sign)
			...
			if (sign.getLine(1).equalsIgnoreCase("[Gate]")) {
				...
				if (sign.getLine(3).equalsIgnoreCase("infinite")
						&& !player.hasPermission("craftbook.mech.gate" + "" + ".infinite")) {
					sign.setLine(3, "0");
				...
				if (!player.hasPermission("craftbook.mech.gate")
						&& !player.hasPermission("craftbook.mech.dgate"))
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		System.out.println("Gate: onBlockBreak: event=" + event + ", protectedBlocks="
				+ protectedBlocks.get(pt));
	...
	public int getBlocks(Sign s, Sign other) {
		...
		if (s.getLine(3).equalsIgnoreCase("infinite") || other != null
				&& other.getLine(3).equalsIgnoreCase("infinite"))
			return 0;
		int curBlocks = 0;
		try {
	...
	public boolean hasEnoughBlocks(Sign s, Sign other) {
		...
		return s != null && s.getLine(3).equalsIgnoreCase("infinite") || other != null
				&& other.getLine(3).equalsIgnoreCase("infinite") || getBlocks(s, other) > 0;
	}

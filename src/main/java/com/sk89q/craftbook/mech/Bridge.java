// $Id$
/*
 * CraftBook Copyright (C) 2010 sk89q <http://www.sk89q.com> This program is
...
public class Bridge extends AbstractMechanic {
	...
	public static class Factory extends AbstractMechanicFactory<Bridge> {
		...
		/**
		 * Explore around the trigger to find a Bridge; throw if things look
		 * funny.
		 * 
		 * @param pt
		...
		public Bridge detect(BlockWorldVector pt) throws InvalidMechanismException {
			...
			if (block.getTypeId() != BlockID.SIGN_POST)
				return null;
			if (!BukkitUtil.toChangedSign(block).getLine(1).equalsIgnoreCase("[Bridge]"))
				return null;
		...
		public Bridge detect(BlockWorldVector pt, LocalPlayer player, ChangedSign sign)
			...
			if (sign.getLine(1).equalsIgnoreCase("[Bridge]")) {
				...
				if (sign.getLine(0).equalsIgnoreCase("infinite")
						&& !player.hasPermission("craftbook.mech.bridge" + "" + ".infinite")) {
					sign.setLine(0, "0");
	...
	private Bridge(Block trigger) throws InvalidMechanismException {
		...
		findBase: {
			...
			if (plugin.getConfiguration().bridgeBlocks.contains(mat)
					&& isValidBridge(proximalBaseCenter, mat, BukkitUtil.toChangedSign(trigger))) {
				break findBase; // On Top
			}

			// If we've reached this point nothing was found on the top, check
			// the bottom
		...
		for (int i = 0; i <= plugin.getConfiguration().bridgeMaxLength; i++) {
			...
			if (farSide.getTypeId() == BlockID.SIGN_POST) {
				...
				if ("[Bridge]".equalsIgnoreCase(otherSignText)
						|| "[Bridge End]".equalsIgnoreCase(otherSignText)) {
					break;
				}
			}
		...
		if (distalBaseCenter.getTypeId() != mat
				&& distalBaseCenter.getData() != proximalBaseCenter.getData()
				|| distalBaseCenter.getRelative(SignUtil.getLeft(trigger)).getTypeId() != mat
				&& distalBaseCenter.getRelative(SignUtil.getLeft(trigger)).getData() != proximalBaseCenter
						.getData()
				|| distalBaseCenter.getRelative(SignUtil.getRight(trigger)).getTypeId() != mat
				&& distalBaseCenter.getRelative(SignUtil.getRight(trigger)).getData() != proximalBaseCenter
						.getData())
			throw new InvalidConstructionException("mech.bridge.material");

		// Select the togglable region
		toggle = new CuboidRegion(BukkitUtil.toVector(proximalBaseCenter),
				BukkitUtil.toVector(distalBaseCenter));
		int left, right;
		try {
		...
		if (CraftBookPlugin.inst().getConfiguration().bridgeProtected) {
			ProtectBlockListener.initialize();
	...
	public void onRightClick(PlayerInteractEvent event) {
		...
		if (!plugin.getConfiguration().bridgeEnabled)
			return;
		...
		if (plugin.getConfiguration().safeDestruction && sign != null
					...
					if (!(event.getPlayer().getGameMode() == GameMode.CREATIVE))
						if (event.getPlayer().getItemInHand().getAmount() <= amount) {
							...
							event.getPlayer().getItemInHand()
									.setAmount(event.getPlayer().getItemInHand().getAmount() - amount);
						}
	...
	public void onBlockRedstoneChange(SourcedBlockRedstoneEvent event) {
		...
		if (!plugin.getConfiguration().bridgeAllowRedstone)
			return;
		...
		if (event.getNewCurrent() == 0) {
			...
			plugin.getServer().getScheduler()
					.scheduleSyncDelayedTask(plugin, new ToggleRegionClosed(null), 2);
		}
	}
	...
	/**
	 * @return whether the bridge can pass through this BlockType (and displace
	 *         it if needed).
	 */
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if (event.getBlock().getTypeId() == BlockID.WALL_SIGN
				|| event.getBlock().getTypeId() == BlockID.SIGN_POST) {
		...
		if (hasEnoughBlocks(sign)) {
			...
			BukkitUtil.toWorld(sign.getLocalWorld()).dropItemNaturally(
					BukkitUtil.toLocation(sign.getBlockVector()), toDrop);
		}
	}

...
public class Door extends AbstractMechanic {
	...
	public static class Factory extends AbstractMechanicFactory<Door> {
		...
		public Door detect(BlockWorldVector pt, LocalPlayer player, ChangedSign sign)
			...
			if (sign.getLine(1).equalsIgnoreCase("[Door Down]")) {
				...
				if (sign.getLine(0).equalsIgnoreCase("infinite")
						&& !player.hasPermission("craftbook.mech.door" + "" + ".infinite")) {
					sign.setLine(0, "0");
		...
		/**
		 * Explore around the trigger to find a Door; throw if things look
		 * funny.
		 * 
		 * @param pt
		 ...
		 *         if the area looked like it was intended to be a door, but it
		 *         failed.
		 */
		@Override
	...
	private Door(Block trigger) throws InvalidMechanismException {
		...
		if (distalBaseCenter.getTypeId() != block
	...
	public void onRightClick(PlayerInteractEvent event) {
		...
		if (!plugin.getConfiguration().doorEnabled)
			return;
	...
	public void onBlockRedstoneChange(SourcedBlockRedstoneEvent event) {
		...
		if (!plugin.getConfiguration().doorAllowRedstone)
			return;
	...
	public boolean hasEnoughBlocks(Sign s) {
		...
		return !(s == null || s.getLine(0) == null)
				&& (s.getLine(0).equalsIgnoreCase("infinite") || getBlocks(s) > 0);
	}
}

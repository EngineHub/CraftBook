 ...
 */
package org.enginehub.craftbook.sponge.mechanics.signcopier;
...
import org.enginehub.craftbook.sponge.mechanics.signcopier.command.EditSignCommand;
...
public class SignCopier extends SpongeMechanic implements DocumentationProvider {
    ...
    private ConfigValue<ItemStack>
            signCopierItem = new ConfigValue<>("signcopier-item", "The item that triggers the Sign Copier mechanic.", ItemStack.builder().itemType(ItemTypes.DYE).add(Keys.DYE_COLOR, DyeColors.BLACK).build(), TypeToken.of(ItemStack.class));

    private SpongePermissionNode
            usePermissions = new SpongePermissionNode("craftbook.signcopier.use", "Allows the user to copy and paste signs.", PermissionDescription.ROLE_USER);

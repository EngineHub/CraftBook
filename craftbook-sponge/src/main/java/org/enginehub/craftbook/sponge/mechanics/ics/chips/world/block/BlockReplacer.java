...
import org.enginehub.craftbook.sponge.util.prompt.BlockStateDataPrompt;
        ...
        if (mode == 0) {
            for (Direction direction : BlockUtil.getDirectFaces()) {
                ...
                if (b.getBlock().equals(blockTypeData.onBlock)) {
                    ...
                    Sponge.getScheduler().createTaskBuilder().delayTicks(delay).execute(() -> replaceBlocks(on, b, traversedBlocks)).submit(
                            CraftBookPlugin.spongeInst().container);

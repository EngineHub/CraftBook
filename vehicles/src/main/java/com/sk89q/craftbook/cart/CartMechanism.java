...
/**
 * Implementers of CartMechanism are intended to be singletons and do all their logic at interation time (like non-persistant mechanics, but allowed
 * zero state even in RAM). In order to be effective, configuration loading in MinecartManager must be modified to include an implementer.
 * 
 * @author hash
 */
public abstract class CartMechanism {
    ...
    /**
     * Called by MinecartManager after either a vehicle move event or a redstone event results in CartMechanismBlocks concluding there is a base block
     * of this CartMechanism's material type involved.
     * 
     * @param cart
     *            if triggered by a move event, the cart involved; if triggered by redstone, a cart in the rail block or null if none.
     * @param blocks
     * @param minor
     *            true if the triggering event is somehow 'minor' (namely, a move event from one part of the same block to another); false otherwise
     *            (i.e. redstone events or move events that cross block boundaries). Most CartMechanism can safely ignore minor events; CartStation is
     ...
     * 
     * @param base
     *            the block on which the rails sit (the type of this block is generally what determines the mechanism type), or null if not
     *            interested.
     * @param rail
     *            the block containing the rails, or null if not interested.
     * @param sign
     *            the block containing the signpost that gives additional configuration to the mechanism, or null if not interested.
    ...
    /**
     * @param rail
     *            the block we're searching for carts (mostly likely containing rails generally, though it's not strictly relevant).
     * @return a Minecart if one is found within the given block, or null if none found. (If there is more than one minecart within the block, the
